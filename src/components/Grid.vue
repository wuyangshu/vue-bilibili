<template>
<div>
<div class="grid" v-for="item in recommendList">
...
export default {
  ...
  computed: {
    ...
    created () {
         this.axios.get('https://api.imjad.cn/bilibili/v2/?get=rank&content=0')
            .then (res => {    
                this.$store.state.recommendList = res.data.rank.list;
         })
    }
}
</script>
...
.grid-title-container .mu-icon-menu {
    float: right;
    height: .56933rem;
    width: .56933rem;
...
.grid-title-container .mu-icon {
    font-size: 1.1rem;
    cursor: inherit;
}
.grid-title-container .mu-icon-button {
    position: absolute;
    z-index:1000;
    right:-0.5rem;
    top:1.1rem;
